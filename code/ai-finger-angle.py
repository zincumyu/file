...
#test开合检测
def get_str_close(up_fingers, list_lms):
    angele_test=120
    if len(up_fingers) >=3:
        #食指
        f_s_s =list_lms[0] -list_lms[5]
        f_s_x =list_lms[6] -list_lms[5]
        angle_f =get_angle(f_s_s,f_s_x)

        #无名指
        w_s_s =list_lms[0] -list_lms[17]
        w_s_x =list_lms[18] -list_lms[17]
        angle_w =get_angle(w_s_s,w_s_x)

        if angle_f < angele_test and angle_f <angele_test:
            print("闭合食指角度{}，无名指角度{}".format(angle_f,angle_w))
        elif angle_f > angele_test and angle_f >angele_test:
            print("打开食指角度{}，无名指角度{}".format(angle_f,angle_w))
        else:
            print("?")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            #打印闭合
            get_str_close(up_fingers, list_lms)

            cv2.putText(img, ' %s' % (str_guester), (90, 90), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 0, 0), 4,
                        cv2.LINE_AA)
